...
from data_baseline_freeze_2 import get_data_loaders_clip
...
def train_epoch_last_block(train_loader, model, optimizer, num_classes, device, text_features, epoch, mfd_edl):
    """
    ...
    for images, labels in progress_bar:
        ...
        out = mfd_edl(v_cls, v_patches, text_features)
        loss = edl_loss(out["alpha"], labels)
        ...
        if not sample_logged:
            ...
            # EDL sanity: strength & uncertainty
            S = out["S"]  # [B,1]
            u = float(num_classes) / S  # [B,1]
            print("EDL S (mean/min/max):", float(S.mean()), float(S.min()), float(S.max()))
            print("EDL u=C/S (mean/min/max):", float(u.mean()), float(u.min()), float(u.max()))

            # Global vs local quick peek
            with torch.no_grad():
                # recompute A_cls / A_local_topk just for stats (same math as inside head)
                A_cls = v_cls @ text_features.T
                A_local = v_patches @ text_features.T
                k_eff = min(getattr(mfd_edl, "k", 16), A_local.size(1))
                A_ltopk = A_local.topk(k_eff, dim=1).values.mean(dim=1)
                A_comb = mfd_edl.gamma * A_cls + (1 - mfd_edl.gamma) * A_ltopk
                logits = A_comb / mfd_edl.tau
                for name, T in [("A_cls", A_cls), ("A_ltopk", A_ltopk), ("logits", logits)]:
                    print(f"{name} stats min/mean/max/std:",
                          float(T.min()), float(T.mean()), float(T.max()), float(T.std()))
                print(f"k_eff={k_eff}, gamma={mfd_edl.gamma}, tau={mfd_edl.tau}")

                top3 = torch.topk(logits, k=3, dim=1)
                print("Label:", labels[:5].tolist())
                print("Pred :", preds[:5].tolist())
...
@torch.no_grad()
def evaluate_last_block(val_loader, model, device, num_classes, text_features, mfd_edl):
    """
    ...
    with torch.inference_mode():
        for images, labels in val_loader:
            ...
            # get CLS + patches (normalized)
            v_cls, v_patches = encode_image_tokens_vit(model, images, require_grad=False)
            out = mfd_edl(v_cls, v_patches, text_features)  # dict: logits, alpha, p_hat, S
            loss = edl_loss(out["alpha"], labels)  # replaces CE
...
class MFDEvidentialHead(nn.Module):
    def __init__(self, k=16, gamma=0.3, tau=0.07, evidence_fn="softplus"):
        super().__init__()
        self.k = k
        self.gamma = gamma
        self.tau = tau
    ...
    def _evidence(self, x):
        ...
        if self.evidence_fn == "relu":
            return F.relu(x)
        return F.softplus(x)  # stable default

    def forward(self, v_cls, v_patches, text_features):
        """
        v_cls:     [B, D]      (L2-normalized)
        v_patches: [B, N, D]   (L2-normalized)
        text_features: [C, D]  (L2-normalized)
        Returns:
            dict with:
              - logits:   [B, C]  (for logging)
              - alpha:    [B, C]  (Dirichlet params)
              - p_hat:    [B, C]  (alpha / S)
              - S:        [B, 1]  (Dirichlet strength)
        """
        # global/local affinities
        A_cls   = v_cls @ text_features.T                # [B, C]
        A_local = v_patches @ text_features.T            # [B, N, C]
        k_eff   = min(self.k, A_local.size(1))
        A_ltopk = A_local.topk(k_eff, dim=1).values.mean(dim=1)  # [B, C]

        # combine (same as MFD), keep logits for metrics/compat
        A_comb  = self.gamma * A_cls + (1 - self.gamma) * A_ltopk
        logits  = A_comb / self.tau

        # ----- Evidential part -----
        # evidence from global & local separately, then fuse like in MFD
        e_cls   = self._evidence(A_cls   / self.tau)     # [B, C]
        e_ltopk = self._evidence(A_ltopk / self.tau)     # [B, C]
        e       = self.gamma * e_cls + (1 - self.gamma) * e_ltopk

        alpha = e + 1.0                                  # Dirichlet params
        S     = alpha.sum(dim=1, keepdim=True)           # strength
        p_hat = alpha / S                                # expected class prob

        return {"logits": logits, "alpha": alpha, "p_hat": p_hat, "S": S}

def edl_loss(alpha, targets):
    # alpha: [B, C], targets: [B] (int labels)
    S = alpha.sum(dim=1, keepdim=True)                  # [B,1]
    log_expect = (alpha / S).clamp_min(1e-8).log()      # log(alpha/S)
    # pick the true-class column and take mean negative log
    loss = -log_expect.gather(1, targets.unsqueeze(1)).mean()
    return loss
...
def train():
    ...
    mfd_edl = MFDEvidentialHead(k=16, gamma=0.3, tau=tau).to(device)

    # 2.3 Optimizer param groups (encoder vs head, with sensible WD)
    enc_lr = config["learning_rate"]  # e.g., 5e-5 or 1e-4
    ...
    # Image Encoder: [B, 1+N, D] CLS + patches
